...
The browser app additionally has eight Node.js tests for the ray model, material presets, file handling and synthetic scene reconstruction. Four Playwright workflows exercise desktop/mobile rendering, material edits, plane selection, interface zoom, expanded dialogs, correction and export, invalidation after parameter edits, scene edits, and independent sensor-station configurations. Local screenshots of the designer, atlas, cloud view and scene lab were inspected. The Pages workflow runs the browser tests before deploying.
