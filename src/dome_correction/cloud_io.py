...
def read_cloud(path):
    ...
    if kind == ".csv":
        ...
        if len(rows) != int(metadata["POINTS"][0]):
            raise ValueError("PCD POINTS does not match data rows.")
        if len(rows) != int(metadata["WIDTH"][0]) * int(metadata["HEIGHT"][0]):
...
def write_cloud(path, cloud, points):
    ...
    with path.open("w", encoding="utf-8", newline="") as stream:
        if cloud.format == ".csv":
            ...
            names = next(line.split()[1:] for line in cloud.header if line.startswith("FIELDS "))
            for line in cloud.header:
                tokens = line.split()
                if tokens and tokens[0] in ("SIZE", "TYPE"):
                    for name in ("x", "y", "z"):
                        tokens[1+names.index(name)] = "8" if tokens[0] == "SIZE" else "F"
                    line = " ".join(tokens)
                stream.write(line + "\n")
