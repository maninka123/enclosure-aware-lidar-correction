...
The browser app has 13 Node.js tests for analytical physics, LUT angle round trips, interpolation, compatibility, file handling, scene reconstruction and finer-LUT convergence in Scene Lab. Seven Playwright workflows exercise desktop/mobile rendering, material selectors, positive-Z projections, correction/export invalidation, scene editing, renderer controls, dynamic LUT generation, LUT heatmaps, cloud comparison, Scene Lab LUT metrics and stale-table detection. The Pages workflow runs these checks before deployment.
